...
async function createComment(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while creating the comment." }); // Respond with an error
...
async function getMyComments(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching comments." }); // Respond with an error
    next(error); // Pass the error to the next middleware
  }
}
...
async function updateComment(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while updating the comment." }); // Respond with an error
    next(error); // Pass the error to the next middleware
  }
}
...
async function deleteComment(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the comment." }); // Respond with an error
    next(error); // Pass the error to the next middleware
  }
}
